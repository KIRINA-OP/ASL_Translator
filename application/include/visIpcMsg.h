    ...
    class visSocket: public visIpcMsg{
        ...
        public:
        virtual int init() = 0;
        virtual int deliver(const std::string& filename) = 0;
        virtual int receive(const std::string& filename) = 0;
    };
    ...
    class visSocketApp: protected visSocket{    
        ...
        int init();
        int deliver(const std::string& filename);
        int receive(const std::string& filename);
        ~visSocketApp();
    };
